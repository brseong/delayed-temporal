...
For each physical circuit $i$ and input code $q$, the held-out repetitions give an unbiased conditional sample variance $s_{i,q}^2$ around that code's held-out mean. With a uniform distribution over the measured codes, the circuit timing-noise variance is $\sigma_i^2=Q^{-1}\sum_q s_{i,q}^2$. This is the expected conditional variance, not the variance of raw spike times; differences between code means remain signal rather than noise.

For $\phi_{\mathrm{NP}}$, fit an affine time response per circuit on the calibration split and use its absolute fitted slope times the 31-code span as the common physical realization of the model's base identity window. Dividing $\sigma_i$ by that span gives a median $r_t=0.03785$ and an interquartile range $[0.03118,0.04449]$; its observed miss rate is $4.58\times10^{-5}$.

The $\phi_{\mathrm{NL}}$ mean transfer failed held-out functional validation, but its conditional variance at each input code still measures timing variation across repeated trials. Normalizing its uniform-code $\sigma_i$ by the corresponding circuit's calibrated $\phi_{\mathrm{NP}}$ span gives a median $r_t=0.02712$ and an interquartile range $[0.02074,0.03351]$. Using this scale excludes the measured systematic transfer error and is not evidence of calibrated $\phi_{\mathrm{NL}}$ operation.

The held-out conditional variances are not constant over input code. For $\phi_{\mathrm{NP}}$, the median within-circuit Spearman correlation between code and conditional standard deviation is $-0.45$, but its interquartile range $[-0.62,0.11]$ crosses zero and five of sixteen circuits have positive correlation. The median standard deviation over circuits decreases from about $0.31\,\mu\mathrm{s}$ in the lowest code third to $0.26\,\mu\mathrm{s}$ in the middle third and $0.27\,\mu\mathrm{s}$ in the highest third, so a constant variance is a coarse but plausible summary.

For $\phi_{\mathrm{NL}}$, all sixteen circuits have negative code-noise correlation, with median Spearman correlation $-0.87$ and interquartile range $[-0.89,-0.85]$. The median standard deviation over circuits decreases from about $0.24\,\mu\mathrm{s}$ to $0.18\,\mu\mathrm{s}$ and $0.15\,\mu\mathrm{s}$ across the three code thirds. Its constant variance therefore averages a pronounced input dependence and must be identified as a homoscedastic approximation.
