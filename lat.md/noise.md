...
For $\phi_{\mathrm{NP}}$, fit an affine time response per circuit on the calibration split and use its absolute fitted slope times the 31-code span as the common physical realization of the model's base identity window. The current formal artifact gives a median $r_t=0.03690$ and an interquartile range $[0.03017,0.03977]$; its observed miss rate is $4.58\times10^{-5}$.

The $\phi_{\mathrm{NL}}$ mean transfer failed held-out functional validation, but its validation residual at each input code still measures timing variation across repeated trials. Normalizing that residual by the corresponding circuit's calibrated $\phi_{\mathrm{NP}}$ span gives a median $r_t=0.02508$ and an interquartile range $[0.01897,0.03168]$. Using this scale excludes the measured systematic transfer error and is not evidence of calibrated $\phi_{\mathrm{NL}}$ operation.
