...
[[utils/transforms/functions.py#gelu_approximation]] uses the cubic tanh approximation but consumes the normalized division result directly as its $[0,1]$ gate: the affine tanh map and the following half scaling cancel exactly. Its dynamic products remain `x^2`, `x^3`, and the gated output. Standalone [[utils/transforms/functions.py#tanh]] remains an implementation support path for encoder poolers and a classification head, but it is not part of the manuscript composition table. [[utils/transforms/functions.py#gelu_approximation_sigmoid]] and [[utils/transforms/functions.py#swiglu_function]] clamp their completed sigmoid-like gates to $[0,1]$ before downstream multiplication; Gaussian gate excursions are recorded without widening the propagated product domains.

The comparison implementation in [[scripts/analysis/gelu_cubic_phi_nl_vit.py#phi_nl_psi_ed_cube]] replaces only the two dynamic products that construct $x^3$. It encodes positive and negative magnitudes with $3\tau_s$, compares each latency with the shared domain upper endpoint through $\psi_{\mathrm{ED}}$ at $\tau_s$, and restores the fixed magnitude scale before the unchanged GELU gate.
