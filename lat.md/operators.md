...
[[utils/transforms/functions.py#gelu_approximation]] uses the cubic tanh approximation but consumes the normalized division result directly as its $[0,1]$ gate: the affine tanh map and the following half scaling cancel exactly. [[utils/transforms/functions.py#gelu_cubic_power_operator]] constructs the signed cubic through the Power composition, so the gated output is the only dynamic multiplication in the maintained GELU. Standalone [[utils/transforms/functions.py#tanh]] remains an implementation support path for encoder poolers and a classification head, but it is not part of the manuscript composition table. [[utils/transforms/functions.py#gelu_approximation_sigmoid]] and [[utils/transforms/functions.py#swiglu_function]] clamp their completed sigmoid-like gates to $[0,1]$ before downstream multiplication; Gaussian gate excursions are recorded without widening the propagated product domains.

The archived comparison in [[scripts/analysis/gelu_cubic_phi_nl_vit.py#gelu_with_multiplication_cube]] replaces only the signed cubic with the earlier two-multiplication construction. Its Power condition delegates to the canonical operator rather than maintaining another implementation.

Completed GELU values use [[calibration#Layer-wise Calibration#Frozen Execution#Fixed GELU Output Bounds]] instead of passing through the last product's symmetric interval. This does not add output calibration or remove intermediate encoding domains.

[[bounds-audit#2026-09-14 Bound Corrections]] applies fixed Swish endpoints, a nonnegative GELU square interval, and synchronized exponential input caps. The square limit is applied only where both nominal inputs are the same; generic multiplication remains signed. Saturated or numerically constant exponential inputs reuse the established full cap for encoding instead of creating a zero-width time window.

The pretrained activation function is invariant to the positive physical time constant. The standalone tanh path pre-scales its exponential argument by $2\tau_s$, sigmoid-GELU by $1.702\tau_s$, and SwiGLU by $\beta\tau_s$; exponential decoding divides by the same $\tau_s$. Thus these paths retain $\tanh(x)$, $\operatorname{sigmoid}(1.702x)$, and $\operatorname{sigmoid}(\beta u)$ at every valid temporal scale. Softmin keeps $\tau_s$ as an intentional normalization temperature instead.
