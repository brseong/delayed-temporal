...
[[scripts/experiments/run_vit_bss2_depth_condition.py#main]] evaluates exactly 500 or 5,000 examples, fixes float64 and deadline-margin ratio 4, and records block-prefixed Gaussian counters. $k=0$ is one deterministic reference; each $k\in\{1,\ldots,12\}$ is evaluated for both measured conditions and seeds 0--2.

### Campaign Protocol

The campaign releases the fixed 5,000-image phase only after all 73 pilot runs satisfy the engineering and identity gates.

[[scripts/experiments/run_vit_bss2_depth_campaign.py#main]] schedules only GPUs 4--7, resumes exact matching conditions, and rejects missing or mixed identities. Pilot accuracy does not select a block or alter the formal grid.

### Summary Artifacts

The reducer reports the cumulative curves without assigning a causal failure to one block.

[[scripts/analysis/summarize_vit_bss2_depth.py#main]] writes `depth_noise_raw.csv`, `depth_noise_summary.csv`, a summary manifest, and PDF/PNG figures. Curves show the three-seed mean and 95% Student-$t$ interval; the clean reference is deterministic.

### Verification

Pure-Python verification covers the scope, fixed condition grid, exact evaluation prefixes, and artifact rejection rules.
...
The campaign must reject missing seeds, invalid block prefixes, mixed dataset identities, and modified hardware summary values.
