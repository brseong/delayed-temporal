...
The evaluator uses [[scripts/runtime/identity.py#model_state_sha256]] after configured parameter perturbations and includes that digest in the run identity. A local checkpoint path is hashed before preflight and checked again before workers start, so file or loaded state changes cannot cross the contract boundary. The identity also binds the activation, attention implementation, complete model configuration digest, preprocessing digest, timing and clock controls, frozen calibration settings, parameter noise controls, and diagnostic controls that can change evaluator behavior.

Each shard writes raw signed 64-bit predictions with a fixed byte order, its exact sample and batch interval, task counts, Gaussian and clamp statistics, run identity, and verified generator contract. [[scripts/experiments/run_exact_sharded_vit.py#merge_shard_results]] rejects coverage gaps, overlaps, identity differences, prediction corruption, generator discontinuities, and statistic site differences before concatenating predictions in global order and reconstructing accuracy, digest, and counters. Interval values, task counts, and integer statistics must be exact nonnegative integers; numeric lookalikes are rejected. Every Gaussian site has the complete event, deadline, resolution, and output counter schema, while every clamp site has exactly value, underflow, and overflow counts. Counter relationships must also be possible under the instrumentation: event subsets cannot exceed events, and mutually exclusive lower and upper saturation counts cannot exceed their shared total.

[[scripts/experiments/run_exact_sharded_vit.py#main]] is the canonical launch and merge entry point. The prefix must contain two full batches, and device availability is checked before and after preflight. After the preflight worker exits, the runner keeps the same device memory and utilization thresholds but permits bounded telemetry resampling so residual activity from that worker is not mistaken for external occupancy; persistent occupancy still stops the run before either shard starts. Every evaluator runs in an isolated process group with forwarded termination signals and bounded cleanup. Managed children inherit an unblocked termination signal mask; completed processes leave the live registry immediately, and signaling skips every child that has exited.
