...
def read_manifest(path: Path) -> list[ExpectedRun]:
    ...
    for row in rows:
        ...
        if axis == "gaussian" and seed is None:
            raise ValueError("Gaussian manifest rows require a timing-noise seed")
        if axis != "gaussian" and seed is not None:
            raise ValueError(f"{axis} manifest rows must not declare a seed")
    ...
    # Lock the intended experimental design: one baseline, singleton mismatch
    # points, and a common seed set repeated at every Gaussian magnitude.
    ...
    gaussian_seeds: dict[float, set[int]] = defaultdict(set)
    for run in expected:
        if run.axis == "gaussian":
            assert run.seed is not None
            gaussian_seeds[run.magnitude].add(run.seed)
    if not gaussian_seeds or len({frozenset(v) for v in gaussian_seeds.values()}) != 1:
        raise ValueError("every Gaussian magnitude must use the same seed set")
    if len(next(iter(gaussian_seeds.values()))) < 2:
        raise ValueError("Gaussian confidence intervals require at least two seeds")
    return expected
...
def parse_run_log(expected: ExpectedRun, log_dir: Path) -> ParsedRun:
    ...
    Gaussian runs must report the configured fraction, derived absolute scale,
    seed, and at least one per-site physical-statistics line. Baseline and static
    mismatch runs intentionally carry no Gaussian counters.
...
def aggregate_runs(runs: Sequence[ParsedRun]) -> list[dict[str, object]]:
    ...
    Gaussian accuracy receives a two-sided 95% Student-t confidence interval over
    timing-noise seeds. Physical rates pool raw counts across replicas. Baseline and
    fixed static mismatch points remain singleton measurements with blank intervals.
    ...
    for (axis, magnitude), replicas in sorted(
        ...
        if axis == "gaussian":
            if len(replicas) < 2:
                raise ValueError("Gaussian groups require at least two replicas")
...
def write_csv_files(
    ...
    *,
    ...
    raw_fields = (
        ...
        "events", "misses", "miss_rate", "outputs", "output_underflows",
...
def plot_results(
    ...
    *,
    figure_prefix: Path,
    model_label: str = "ViT-S",
    archive_existing: bool = True,
) -> None:
    """Render Gaussian robustness and an optional static-mismatch panel.

    Gaussian points display the seed mean and 95% Student-t interval. Static
    mismatch is a fixed seed-42 realization and therefore appears without an error
    band. Both panels share the same deterministic 5,000-image baseline.
    ...
    if mismatch_rows:
        # The mismatch panel intentionally has no uncertainty band: it is one fixed
        # threshold-offset realization, matching the declared experiment contract.
    ...
    axes[0].set_ylabel("ImageNet top-1 accuracy (5k subset)")
...
def parse_arguments(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--model-label", default="ViT-S")
    parser.add_argument(
        "--archive-existing",
