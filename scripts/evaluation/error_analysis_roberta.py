...
def evaluate_roberta_model(args: Arguments) -> None:
    ...
    # Convert each relative CLI scale at its encoder boundary; both overrides share
    # one seeded generator and one deadline-margin ratio.
    ...
    if (gaussian_enabled or calibration_mode is not None) and use_data_parallel:
        raise RuntimeError(
            "Gaussian spike-time noise and calibration do not support DataParallel; "
    ...
    wandb.init(project="roberta-evaluation", config=cfg, name=args.experiment_name)
    ...
    if calibration_mode is not None:
        ...
        calibration_state = run_text_calibration(
            model, args, model_family="roberta",
