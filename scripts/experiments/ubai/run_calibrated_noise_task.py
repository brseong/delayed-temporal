...
def check_hash(path: Path, expected: str) -> None:
    expected = identity.checked_hash(expected)
...
def runtime_values(deployment: dict[str, Any]) -> list[str]:
    ...
    values = [str(runtime_files.absolute_path(runtime[name])) for name in path_fields]
    ...
    values += [runtime['env_archive_sha256'], str(unpacked_bytes),
               str(runtime_files.absolute_path(runtime['container_image'])), runtime['container_image_sha256'],
               str(scratch_bytes)]
    return values
...
def validate_contract(deployment: dict[str, Any], manifest: Path) -> tuple[dict[str, Any], list[Any], list[dict[str, str]]]:
    ...
    for name, expected in (
        ...
        if runtime_files.absolute_path(deployment[name]) != expected:
    ...
    for relative, expected in deployment.get('runtime_library_sha256s', {}).items():
        path = REPOSITORY_ROOT / relative
        if not path.is_relative_to(REPOSITORY_ROOT / 'scripts/runtime'):
            raise ValueError('Runtime library identity must stay inside scripts/runtime')
    ...
    declared_manifest = runtime_files.absolute_path(deployment['assigned_manifest_path'])
    ...
    for tool in deployment.get('runtime_tools', []):
        tool_path = runtime_files.absolute_path(tool['path'])
...
def check_assets(deployment: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for asset in deployment.get('assets', []):
        path = runtime_files.absolute_path(asset['path'])
...
def execute(deployment_path: Path, deployment: dict[str, Any], manifest: Path, task_index: int) -> None:
    ...
    with lock_path.open('a') as lock:
        ...
        if log_path.exists():
            ...
            return
        local_gpu.require_single_gpu(PYTHON, 'ubai')
        deployment_sha = identity.sha256_file(deployment_path)
        ...
        try:
            with partial.open('x', encoding='utf-8') as handle:
                ...
                runtime_files.atomic_json(status_path, {**status, 'state': 'running', 'pid': child.pid})
            ...
            runtime_files.atomic_json(status_path, {**status, 'state': 'complete', 'finished_at': time.time(),
            ...
            runtime_files.atomic_json(
                status_path,
                {**status, 'state': 'failed', 'finished_at': time.time(), 'error': str(error)},
            )
            raise
        finally:
            if child is not None and child.poll() is None:
...
def main() -> None:
    ...
    if args.runtime_values:
        ...
        return
    manifest = args.manifest or runtime_files.absolute_path(deployment['assigned_manifest_path'])
    if args.check_only:
        check_python_version(deployment)
        ...
        receipt = {
            'deployment_sha256': identity.sha256_file(args.deployment), 'python': platform.python_version(),
        ...
        runtime_files.atomic_json(EXPERIMENT_ROOT / 'status/ubai-preparation.json', receipt)
