...
def validate_calibration(root: Path) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if evidence["calibration_sha256"] != identity.sha256_file(root / "calibration.json"):
        raise ValueError("Calibration table checksum mismatch")
    if evidence["collection_log_sha256"] != identity.sha256_file(root / "logs/calibration_collect.log"):
    ...
    if identity.sha256_file(wrapper) != experiment["calibration_evaluator_sha256"]:
...
def prepare(root: Path, output: Path, *, image: Path, host_base: Path) -> dict[str, Any]:
    ...
    for name in scripts:
        runtime_files.immutable(output / "code" / name, (REPO / "scripts/experiments/ubai" / name).read_bytes())
    runtime_tools = []
    for original, relative in (
        ...
        runtime_tools.append({"path": "/calibrated-deployment/" + relative,
                              "sha256": identity.sha256_file(output / relative)})
    runtime_files.immutable(output / "manifests/pending.tsv", serialize(fields, candidates))
    ...
    for relative in ("experiment.json", "calibration.json", "calibration-evidence.json", "manifests/grid.tsv", "logs/calibration_collect.log", "logs/dense_reference.log"):
        runtime_files.immutable(output / "experiment" / relative, (root / relative).read_bytes())
    clean = root / "logs/clean_spiking_baseline.log"
    clean_hash = None
    if clean.exists():
        runtime_files.immutable(output / "experiment/logs/clean_spiking_baseline.log", clean.read_bytes())
        clean_hash = identity.sha256_file(clean)
    ...
    deployment = {
        ...
        "calibration_evidence_sha256": identity.sha256_file(root / "calibration-evidence.json"),
        "grid_sha256": identity.sha256_file(root / "manifests/grid.tsv"),
        "assigned_manifest_path": "/calibrated-deployment/manifests/pending.tsv",
        "assigned_manifest_sha256": identity.sha256_file(output / "manifests/pending.tsv"),
        "task_script_sha256": identity.sha256_file(output / "code/calibrated_noise_task.sbatch"),
        "prep_script_sha256": identity.sha256_file(output / "code/calibrated_noise_prep.sbatch"),
        "worker_sha256": identity.sha256_file(output / "code/run_calibrated_noise_task.py"),
        "runtime_library_sha256s": {
            str(path.relative_to(REPO)): identity.sha256_file(path)
            for path in (
                REPO / "scripts/runtime/files.py",
                REPO / "scripts/runtime/identity.py",
                REPO / "scripts/runtime/local_gpu.py",
            )
        },
    ...
    runtime_files.immutable_json(output / "deployment.json", deployment)
    return deployment
