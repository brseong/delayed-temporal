...
def verify_commands_and_admission() -> None:
    for family in ("bert", "roberta", "gpt2"):
        ...
        for gpu in (4, 5, 6, 7):
            runner.validate_gpu_request(gpu, "baekryun-cuda129")
    for gpu in (0, 1, 2, 3, 8, -1, True, "4"):
        rejects(runner.validate_gpu_request, gpu, "baekryun-cuda129")
    rejects(runner.validate_gpu_request, 4, "gate1.hpc")
