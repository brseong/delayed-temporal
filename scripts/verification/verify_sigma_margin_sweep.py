...
def verify_slurm_contract() -> None:
    ...
    assert "SLURM_TMPDIR" not in task and 'enroot_data="/enroot/$(id -u)/data"' in task
    ...
    assert "SLURM_TMPDIR" not in reducer and 'enroot_data="/enroot/$(id -u)/data"' in reducer
    assert '$runtime_parent/dt:/opt/conda/envs/dt' in reducer
