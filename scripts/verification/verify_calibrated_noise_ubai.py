...
class PreparationTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.experiment = {
            ...
            "calibration_evaluator_sha256": prepare.sha256(wrapper),
            "evaluator_path": str(evaluator.relative_to(self.source)),
            "evaluator_sha256": prepare.sha256(evaluator),
    ...
    def refresh_evidence(self) -> None:
        ...
        self.evidence = {
            "experiment_sha256": prepare.sha256(self.root / "experiment.json"),
            "calibration_sha256": prepare.sha256(self.root / "calibration.json"),
            "collection_log_sha256": prepare.sha256(self.root / "logs/calibration_collect.log"),
            "sites": 48,
        }
    ...
    def test_immutable_creation_replay_and_rejection(self) -> None:
        ...
        prepare.immutable(target, b"original")
        prepare.immutable(target, b"original")
        with self.assertRaises(ValueError):
            prepare.immutable(target, b"changed")
    ...
    def test_worker_contract_and_manifest_numeric_equalities(self) -> None:
        ...
        for field in ("time_noise_std_abs", "deadline_margin_abs"):
            ...
            deployment["grid_sha256"] = prepare.sha256(manifest)
            with self.assertRaises(ValueError, msg=field):
        ...
        deployment["assigned_manifest_sha256"] = prepare.sha256(pending_path)
        with self.assertRaises(ValueError):
            self.validated_contract(deployment)
    ...
    def test_worker_rejects_hash_and_fixed_floor_changes(self) -> None:
        ...
        for field, path in (("experiment_sha256", "experiment.json"), ("calibration_evidence_sha256", "calibration-evidence.json")):
            deployment[field] = prepare.sha256(self.root / path)
        with self.assertRaises(ValueError):
            self.validated_contract(deployment)
