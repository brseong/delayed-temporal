...
class LIF_Filter(MemoryModule):
    def __init__(self,
        ...
        self.tau_m = tau_m
        self.tau_s = tau_s
        self.beta_m = tau2beta(tau_m)
        self.beta_s = tau2beta(tau_s)
        
        self.synapse_filter = SynapseFilter(beta=self.beta_s, step_mode=step_mode, learnable=False)
        self.mem_filter = SynapseFilter(beta=self.beta_m, step_mode=step_mode, learnable=False)
        
    def extra_repr(self):
        return super().extra_repr() + f', lif_beta={self.beta_m}, filter_beta={self.beta_s}, step_mode={self.step_mode}, tau_m={self.tau_m}, tau_s={self.tau_s}'    
    ...
    def forward_discrete(self, x):
        ...
        V = (V_pos - V_neg)  # Use inverse of time constant difference for computation efficiency
