...
- Reserve "encoder" for Transformer encoders. Name a potential-to-spike mapping or application a `$\phi$ operator` or `$\phi$ operation`; do not call it an encoder. General coding terms such as TTFS encoding and positional encoding remain valid when they do not name a `$\phi$` mapping.
